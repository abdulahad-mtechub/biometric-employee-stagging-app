...
import {getImageOrientation, compressImageForUpload} from '../../utils/imageOrientation';
...
const AttendenceRequestDetails = ({route, navigation}) => {
  ...
  // Process images for orientation correction
  useEffect(() => {
    if (!item?.punches || !token) return;

    const processImages = async () => {
      const corrections = {};

      for (const punch of item.punches) {
        if (punch.evidence?.selfie_url && !correctedImages[punch.id]) {
          try {
            // Detect orientation - auto-detect based on dimensions when EXIF is stripped
            const result = await getImageOrientation(
              punch.evidence.selfie_url,
              {
                shouldCorrect: true,
                autoDetectRotation: true, // Auto-detect rotation based on image dimensions
              },
            );
            console.log(
              '[AttendenceRequestDetails] Image orientation result:',
              punch.id,
              JSON.stringify(result, null, 2),
            );

            // Use correctedUri if rotation was applied, otherwise use the original
            if (result.correctedUri) {
              corrections[punch.id] = result.correctedUri;
            } else if (result.originalUri) {
              // Even if no rotation needed, we need to compress the original image
              // Store the original path to be compressed later
              corrections[punch.id] = result.originalUri.startsWith('file://')
                ? result.originalUri.replace('file://', '')
                : result.originalUri;
            }
          } catch (error) {
            console.log(
              '[AttendenceRequestDetails] Image correction error:',
              error.message,
            );
          }
        }
      ...
      if (Object.keys(corrections).length > 0) {
        setCorrectedImages(prev => ({...prev, ...corrections}));

        const uploads = {};
        for (const [punchId, correctedPath] of Object.entries(corrections)) {
          try {
            console.log(
              '[AttendenceRequestDetails] Uploading corrected image for punch:',
              punchId,
              'Path:',
              correctedPath,
              'Token available:',
            ...
            // Compress image to ensure < 1MB before upload
            const compressedPath = await compressImageForUpload(correctedPath);
            console.log(
              '[AttendenceRequestDetails] Compressed image path:',
              compressedPath,
            );

            // Add file:// prefix if not present (required for React Native fetch)
            const filePath = compressedPath.startsWith('file://')
              ? compressedPath
              : `file://${compressedPath}`;
            ...
            console.log(
              '[AttendenceRequestDetails] Upload result for punch:',
              JSON.stringify(uploadResult, null, 2),
            );
            if (uploadResult?.data?.url) {
              uploads[punchId] = uploadResult.data.url;
              console.log(
                '[AttendenceRequestDetails] Upload success:',
                uploadResult.data.url,
              );
            }
          } catch (uploadError) {
            console.log(
              '[AttendenceRequestDetails] Upload error:',
              uploadError.message,
            );
          }
        }
        ...
        if (Object.keys(uploads).length > 0) {
          setUploadedUrls(prev => ({...prev, ...uploads}));
        }
      }
    };
              ...
              <View style={styles.detailSection}>
                ...
                {imageErrors[punch.id] ? (
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      const imageUri =
                        uploadedUrls[punch.id] ||
                        correctedImages[punch.id] ||
                        punch.evidence.selfie_url;
                      setImageViewerUrl(imageUri);
