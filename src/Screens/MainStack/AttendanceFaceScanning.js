...
import FaceDetector, {
  FACE_STATUS,
} from "../../components/FaceDetection/FaceDetector";
...
const AttendanceFaceScanning = ({ route }) => {
  ...
  // Handle capture from FaceDetector — API image quality, then existing upload + postValidateFace
  ...
  const handleDetectionSuccess = async (imagePath) => {
    ...
    try {
      // Use raw captured image directly - skip ImageResizer to avoid rotation issues
      ...
      console.log("Image payload:", JSON.stringify(uploadPath, null, 3));
      const uploadResponse = await uploadImage(imagePayload, token);
      console.log(
        "Image upload response:",
        JSON.stringify(uploadResponse, null, 3),
      );
      ...
      console.log(
        "Face validation response:",
        JSON.stringify(responseValidateFace, null, 3),
      );
  ...
  const renderCustomStatus = ({ faceStatus, getStatusMessage, photoUri }) => {
    // Don't show status badge when photo is captured
    if (photoUri) {
      return null;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Processing Modal — photo quality vs face / registration verification */}
      <Modal
...
export default AttendanceFaceScanning;
